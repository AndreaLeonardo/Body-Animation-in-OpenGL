...
DebugSkinningTechnique::DebugSkinningTechnique()
{
}

bool DebugSkinningTechnique::Init()
{
    if (!Technique::Init()) {
        return false;
    }

    if (!AddShader(GL_VERTEX_SHADER, "debug.vs")) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, "debug.fs")) {
        return false;
    }
...
void DebugSkinningTechnique::SetWVP(const Matrix4f& WVP)
{
    glUniformMatrix4fv(WVPLoc, 1, GL_TRUE, (const GLfloat*)WVP.m);
}


void DebugSkinningTechnique::SetTextureUnit(unsigned int TextureUnit)
{
    glUniform1i(samplerLoc, TextureUnit);
}

void DebugSkinningTechnique::SetSpecularExponentTextureUnit(unsigned int TextureUnit)
{
    glUniform1i(samplerSpecularExponentLoc, TextureUnit);
}


void DebugSkinningTechnique::SetDirectionalLight(const DirectionalLight& Light)
{
...
void DebugSkinningTechnique::SetCameraLocalPos(const Vector3f& CameraLocalPos)
{
    glUniform3f(CameraLocalPosLoc, CameraLocalPos.x, CameraLocalPos.y, CameraLocalPos.z);
}


void DebugSkinningTechnique::SetMaterial(const Material& material)
{
...
void DebugSkinningTechnique::SetPointLights(unsigned int NumLights, const PointLight* pLights)
{
...
void DebugSkinningTechnique::SetSpotLights(unsigned int NumLights, const SpotLight* pLights)
{
...
void DebugSkinningTechnique::SetDisplayBoneIndex(uint DisplayBoneIndex)
{
